...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $konfirmasi_password = htmlspecialchars($_POST['konfirm_password'], ENT_QUOTES, 'UTF-8');
    $role = isset($_POST['role']) ? htmlspecialchars($_POST['role'], ENT_QUOTES, 'UTF-8') : '';

    // Validasi input
    if (empty($name) || empty($email) || empty($no_hp) || empty($password) || empty($konfirmasi_password)) {
